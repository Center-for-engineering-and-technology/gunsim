...
class AbstractInternalFilter { // abstract band pass internal filter structure
protected: 
	struct DFT { // Discrete Fourier transform tools
	...
	struct Window { // the amount of most common window types
		enum struct WindowType { RECTANGULAR, HANN, HAMMING, BLACKMAN } window_type;
...
public:
	...
	const std::vector<double>& getImpulse() const { return imp_data; }
