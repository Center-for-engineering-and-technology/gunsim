...
namespace energy_comp
{
        ...
        for (size_t g = 0; g < energyParams.gunArray.size(); g++) { 
            ...
            for (size_t ag = 0; ag < 2 * energyParams.gunArray.size(); ag++) { 
                lengths[g][ag] = std::sqrt(
                    std::pow(energyParams.gunArray[g].x - energyParams.gunArray[ag % energyParams.gunArray.size()].x, 2) + 
                    std::pow(energyParams.gunArray[g].y - energyParams.gunArray[ag % energyParams.gunArray.size()].y, 2) +
                    std::pow(energyParams.gunArray[g].z - std::pow(-1, ag / energyParams.gunArray.size()) * energyParams.gunArray[ag % energyParams.gunArray.size()].z, 2)
                );
            }
        }
    }
    ...
    void Energy::computeSignalFromFile(size_t g) {
        ...
        std::vector<double> signal(energyOptions.sigParams.sampleNum);
