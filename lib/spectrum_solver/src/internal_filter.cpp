...
AbstractInternalFilter::Window::Window(WindowType window_type_) : window_type(window_type_) {
	switch (window_type) {
	case WindowType::RECTANGULAR: w = [](double n, double m) -> double {
		if (std::abs(n) > (m - 1) / 2.)
			return 0;
		return 1;
	};
	...
	break;
	case WindowType::HAMMING: w = [](double n, double m) -> double {
		if (std::abs(n) > (m - 1) / 2.)
			return 0;
	...
	break;
	case WindowType::HANN: w = [](double n, double m) -> double {
		if (std::abs(n) > (m - 1) / 2.)
			return 0;
	...
	break;
	case WindowType::BLACKMAN: w = [](double n, double m) -> double {
		if (std::abs(n) > (m - 1) / 2.)
			return 0;
...
void AbstractInternalFilter::windowedSincResponce(std::vector<double>& impulse) const {
	...
	std::function<double(double)> sinc = [](double x) -> double { if (std::abs(x) < 1e-6) return 1 - std::pow(x, 2) / 6. + std::pow(x, 4) / 120. - std::pow(x, 6) / 5040.; return std::sin(x) / x; };
