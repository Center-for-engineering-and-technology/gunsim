...
- [Eigen](https://gitlab.com/libeigen/eigen/tree/master)
...
```
Пример заполнения входных данных находится в каталоге `test/spectrum_solver_test/test_data`.
### Применение модели
...
Bandpass-фильтры используются для выделения полезных сигналов в определенном частотном диапазоне, подавляя шумы и помехи за его пределами. Они особенно важны при обработке сигналов от морских пневмоисточников, где необходимо выделить полезные сейсмические данные на фоне шумов.
...
- Частота среза задаётся как процент от частоты Найквиста
...
| `setHighNyqPrcntFreqCut(double)` | `[50, 100]` % | доля от частоты Найквиста (например, `95` → $$f_cut = 0.95 f_Nyq$$) |
...
Настройка свободных параметров ПИ `GunType`, позволяет приближать смоделированную сигнатуру пневмоисточника к сигнатуре конкретного типа (напр. 1500С, 1900LLX, ...). Для этой цели реализован функционал, позволяющий на основе исходных данных о сигнатурах ПИ при различных начальных условиях (объем камеры, стартовое давление, глубина погружения) на основе **генетического алгоритма** искать подходящий набор параметров пушки. 

### Основные классы

#### `Mesh` и `Data`

Описывают сетку задаваемых параметров: h -- глубина, v -- объем, р -- давление. Имеется метод `parseData()` для парсинга исходных сигнатур.
...
struct Options {
    ...
    double MUTATION_RATE = 0.1; // вероятность мутаций
...
Вкратце, имеется несколько наборов оптимизируемых параметров (особи) с правилами расчета целевой функции (отличие модельных сигнатур от исходных данных). Особи объединяются в популяцию и сортируются по значениям целевой функции. Затем производится случайная селекция из лучших экземпляров, которые затем скрещиваются по определенным правилам (кроссинговер) для формирования новой популяции. Спустя несколько поколений лучшие экземпляры в популяции рассматриваются в качестве искомых параметров.
...
```
Метод `genome()` возвращает объект типа `Genome`, обладающий списком оптимизируемых данных конкретной особи, значением целевой функции и методами для мутации и кроссинговера.
